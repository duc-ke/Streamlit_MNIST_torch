...
def main():
    ...
    st.title("PyTorch DL Training 🧠🏋️‍♂️")
    st.write("This app trains a simple CNN on the MNIST dataset.")

    # Set hyperparameters
    ## 사이드바 헤더 설정
    st.sidebar.markdown("# Training 🧠🏋️‍♂️")
    st.sidebar.subheader("Hyperparameters setting")
    ## input을 딱 잡는게 아니라 사이드바로 구성
    batch_size = st.sidebar.slider("Batch size", min_value=1, max_value=128, value=64, step=4)
    epochs = st.sidebar.slider("Epochs", min_value=1, max_value=100, value=5, step=1)
    learning_rate = st.sidebar.slider("Learning rate", min_value=0.0001, max_value=0.1, value=0.01, step=0.001, format="%.4f")
    ...
    if st.session_state.train_button_clicked and not st.session_state.trained:
        with st.spinner("Training the model..."):
            ...
            st.info("Loss & Accuracy history graph and dataframe")
            col1, col2 = st.columns(2)
            chart_container = col1.empty()
            results_container = col2.empty()

            # visualization of samples
            st.info("Sample visualization of the current epoch")
